    ...
    if (oParams.exist(PARAM_URL_KEY)):
        ...
        if (oParams.exist(PARAM_PAGE_KEY)):
            ...
            if (sRootUrl.startswith(URL_MOVIES_ALL) |
                ...
                return
            _parseMedia(sUrl,sRootUrl,iPage,sPattern)
        else:
...
def _parseMedia(sUrl,sRootUrl,iPage,sPattern):
    logger.error("parse %s with pattern %s" % (sUrl, sPattern))

    oRequestHandler = cRequestHandler(sUrl)
    sHtmlContent = oRequestHandler.request()

    _parseMovie(sHtmlContent,sUrl,sRootUrl,iPage,sPattern)

def _parseMovie(sHtmlContent,sUrl,sRootUrl,iPage,sPattern):
    oGui = cGui()
    ...
    if aResult[0]:
        for aEntry in aResult[1]:
            ...
            oGuiElement.setSiteName(SITE_IDENTIFIER)
            serie = False
            if (sRootUrl.startswith(URL_SERIES) | _mediaIsASerie(newUrl)):
                serie = True
...
def _search(oGui, sSearchText):
    ...
    _parseMedia(sUrl,sUrl,1,PATTERN_LIST)
